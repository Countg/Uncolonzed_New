...
export default function ContactForm() {
  ...
  const onSubmitHandler = (data, e) => {
    e.preventDefault();
    axios
      .post('https://uncolonized-site.herokuapp.com/send', data)
      .then((res) => {
        setSent(true);
        setSuccess(true);
      })
    ...
    <div className='p-8 w-full'>
      <form onSubmit={handleSubmit(onSubmitHandler)} reset={reset}>
        ...
        <div className='flex justify-end py-4'>
          <button
            type='submit'
            className='mb-20 md:mb-0 px-8 py-2 rounded-md bg-white shadow-lg text-xl font-Arimo font-semibold flex flex-row space-x-4 items-center dark:text-gray-700'>
            Submit
          </button>
        </div>
